...
export const NavItem = ({ children, label, icon = null, isActive = false, child = false, className = null, to = null }) => {
    ...
    return (
        <NavLink to={ to } onClick={ !child && toggle } className={`${children? 'h-fit':'h-[6vh]'} text-[#A4A6A8] flex flex-col cursor-pointer`}>
            <div className={`${isActive? 'bg-[#383E49] text-white':'bg-transparent'} rounded-[0.5vw] ${child? 'hover:bg-transparent w-fit':'hover:bg-[#383E49]'} hover:text-white w-full h-full px-[1vw] flex justify-between items-center`}>
                <div className='flex items-center gap-[0.8vw] h-[6vh]'>
                    { icon }
                    <span className="text-[1vw] font-inter font-medium">{label}</span>
                </div>
                { children && <CaretIcon className={ `transition ${open && 'rotate-90'}` } /> }
            </div>
            { children && <div className={ `${(open || active) ? `max-h-screen` : `hidden`} w-full ml-11 p-0 mt-2 relative transition-[max-height] hover:text-blue-600` }>
                { children }
            </div> }
        </NavLink>
