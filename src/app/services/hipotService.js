...
export const hipotService = apiSlice.injectEndpoints({
    endpoints: (builder) => ({
        ...
        getLine1HipotNgCount: builder.query({
            query: () => ({
                url: 'process/line-1/hipot/auto-ng-causes/quantity-ng'
            }),
            transformResponse: res => res?.data?.qtyNg
        }),
        getLine1HipotOkCount: builder.query({
            query: () => ({
                url: 'process/line-1/hipot/auto-ng-causes/quantity-ok'
            }),
        ...
        line1HipotLogs: builder.query({
            query: (data = []) => ({
                url: `process/line-1/hipot/auto-ng-causes?${Object.keys(data).map((item, i) => `${item}=${data[item]}`).join('&')}`,
            }),
            transformResponse: res => res?.data
        }),
        line1HipotTopManualNg: builder.query({
