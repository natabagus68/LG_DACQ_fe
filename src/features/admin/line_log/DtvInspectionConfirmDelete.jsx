...
import { useLine1DtvInspectionDestroyMutation } from "../../../app/services/dtvInspectionService";
import { TrashIcon } from "../../../common/components/icons";

export default function DtvInspectionConfirmDelete({ dtvInspectionId }) {
    let [isOpen, setIsOpen] = useState(false);
    const [
        _destroyDtvInspection,
        {
            isLoading: destroyDtvInspectionIsLoading,
            isError: destroyDtvInspectionIsError,
        },
    ...
    function destroyDtvInspection(e) {
        e.preventDefault();
        _destroyDtvInspection(dtvInspectionId)?.then((success) => {
            setIsOpen(false);
        });
    }
    ...
    return (
        <>
            <div className="flex items-center justify-center">
                <button
                    onClick={openModal}
                    className="bg-[#F04438] text-white rounded-lg px-2 py-1"
                >
                    <TrashIcon />
                </button>
            </div>
