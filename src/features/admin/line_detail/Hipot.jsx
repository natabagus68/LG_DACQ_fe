...
import { useGetLine1HipotNgCountQuery, useGetLine1HipotOkCountQuery, useGetLine1HipotProcessChartQuery, useGetLine1HipotTopTenLogsQuery, useGetLine1HipotTop5NgCauseQuery, useLine1HipotTopManualNgQuery, useLine1HipotUpdateManualNgMutation } from '../../../app/services/hipotService';
...
const HipotChart = ({ frequent, ppmOn }) => {
    const { data: line1HipotProcessChart = [], isLoading: line1HipotProcessChartLoading } = useGetLine1HipotProcessChartQuery(frequent, {
        pollingInterval: 1000,
    });
    const data = useMemo(() => {
        return {
            labels: line1HipotProcessChart.map(item => item?.x || '-'),
            datas: line1HipotProcessChart.map(item => (item?.y || 0) * (ppmOn ? 10000 : 1))
        };
    }, [line1HipotProcessChart, ppmOn]);
...
const CompAddData = ({ setAlert }) => {
    ...
    const [line1HipotUpdateManualNg, { error: line1HipotUpdateManualNgError, isLoading: line1HipotUpdateManualNgLoading }] = useLine1HipotUpdateManualNgMutation();
    const line1HipotSubmitManualNg = e => {
        e.preventDefault();
        line1HipotUpdateManualNg(description);
    };
    return (
        <form onSubmit={ line1HipotSubmitManualNg }>
            <div className='w-[432px] flex flex-col gap-2'>
                <div className='flex gap-4 items-center'>
                    <NgCauseIcon />
                ...
                <div className='flex flex-col'>
                    <textarea value={ description } onChange={ e => setDescription(e.target.value) } className='outline-none border rounded-xl h-[185px] px-4 py-3' />
                    <div className='flex justify-end'>
                        <span className='text-[12px]'>{ description.length }/500</span>
                    </div>
                </div>
                <div className='flex gap-2'>
                    <button type="button" role="button" onClick={ () => setAlert(false) } className='h-[44px] cursor-pointer flex-1 text-[16px] font-medium border-[1px] flex items-center justify-center rounded-md'>Cancel</button>
                    <button type="submit" className='h-[44px] flex-1 text-[16px] font-medium text-white bg-[#229BD8!important] flex items-center justify-center rounded-md'>Upload</button>
                </div>
            </div>
        </form>
    );
...
export const CompImage = ({ setAlert }) => {
    const selectedLogImage = useSelector(state => state.line1Hipot.selectedLogImage);
    useEffect(() => {
...
const TopAutoNgTable = () => {
    const { data: line1HipotTop5NgCause = [], isLoading: line1HipotTop5NgCauseLoading } = useGetLine1HipotTop5NgCauseQuery(null, {
        pollingInterval: 1000,
    });
    return (
        <Table>
            <Table.Thead>
                <Table.Tr>
                    <Table.Th className={ `bg-[#E2F1FF] py-2 text-sm` } order={ false }>Model</Table.Th>
                    <Table.Th className={ `bg-[#E2F1FF] py-2 text-sm` } order={ false }>NG Cause</Table.Th>
                    <Table.Th className={ `bg-[#E2F1FF] py-2 text-sm` } order={ false }>Timestamp</Table.Th>
                </Table.Tr>
            </Table.Thead>
            <tbody>
                { line1HipotTop5NgCauseLoading && <>
                    <div className="flex flex-1 bg-red-300"></div>
                </> }
                { line1HipotTop5NgCause.map((item, i) => (
                    <Table.Tr key={ i }>
...
const TopManualNgTable = () => {
    const { data: topManualNg, isLoading: topManualNgLoading } = useLine1HipotTopManualNgQuery();
...
export const Hipot = () => {
    ...
    const { data: line1HipotOkCount, isLoading: line1HipotOkCountLoading } = useGetLine1HipotOkCountQuery(null, {
        pollingInterval: 1000,
    });
    const { data: line1HipotNgCount, isLoading: line1HipotNgCountLoading } = useGetLine1HipotNgCountQuery(null, {
        pollingInterval: 1000,
    });
    const { data: line1HipotTopTenLogs = [], isLoading: line1HipotTopTenLogsLoading } = useGetLine1HipotTopTenLogsQuery(null, {
        pollingInterval: 1000,
    });
            ...
            <div className='relative h-full p-6 flex font-inter flex-col gap-4 bg-white'>
                <div className="text-[#A9A8A8] flex items-center justify-between mb-3">
                    <div className='flex items-center gap-1'>
                        ...
                        <span className='text-sm'>/</span>
                        <span className="font-semibold text-sm text-[#514E4E]">HIPOT</span>
                    </div>
                </div>
                    ...
                    <Card>
                        <div className='flex flex-col flex-1 gap-1'>
                            <div className="flex items-center justify-between">
                                <span className='font-bold text-lg'>Hipot</span>
                                ...
                                <div className='flex gap-3 items-center'>
                                    ...
                                    <button disabled onClick={ () => setAlert({ bool: true, comp: 'excel' }) } className='flex gap-1 cursor-pointer items-center border-[1px] p-1 rounded-sm'>
                            ...
                            <div className='w-full h-full'>
                                <HipotChart frequent={ frequent } ppmOn={ ppmOn } />
                            </div>
                        </div>
                    </Card>
                </div>
                    ...
                    <div className='col-span-3 flex gap-4 flex-col'>
                        <div className='grid grid-cols-4 gap-4'>
                            <Card className={ `py-[21px] px-[10px]` }>
                                ...
                                <span className='text-[#2D2A2A] m-auto text-[40px] font-bold'>{ line1HipotOkCount }</span>
                            </Card>
                            <Card className={ `py-[21px] px-[10px]` }>
                                <span className='bg-[#FAC5C1] h-[32px] rounded-xl flex items-center justify-center text-[#DE1B1B] text-sm'>Quantity NG</span>
                                <span className='text-[#2D2A2A] m-auto text-[40px] font-bold'>{ line1HipotNgCount }</span>
                            </Card>
                            {/* <Card>
                        ...
                        <div className='flex gap-3 flex-col border rounded-xl py-[19px] px-[24px]'>
                            <div className='flex justify-between pb-1 items-center'>
                                ...
                                <Link to={ 'log' } className='flex gap-1 items-center px-3 py-2 bg-[#229BD8] text-white rounded-md'>
                            ...
                            <Table>
                                ...
                                <tbody>
                                    { line1HipotTopTenLogs.map((item, i) => (
