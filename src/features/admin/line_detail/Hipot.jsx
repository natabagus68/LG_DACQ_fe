...
const HipotChart = ({
    ...
    } = useGetLine1HipotProcessChartQuery(searchParams.frequent, {
        pollingInterval: interval,
    });
    const data = useMemo(() => {
        return {
            labels: line1HipotProcessChart.map((item) => item?.x || "-"),
            datas: line1HipotProcessChart.map(
            ...
            onClick={(event) => {
                ...
                if (lineEl) {
                    let from_date = moment(
                        line1HipotProcessChart?.[lineEl.index]?.x,
                        "hh-mm"
                    )
                        .add(-1, "hour")
                        .startOf("hour")
...
const CompAddData = ({ setAlert }) => {
    ...
    const [
        line1HipotUpdateManualNg,
        {
            error: line1HipotUpdateManualNgError,
            isLoading: line1HipotUpdateManualNgLoading,
        },
    ] = useLine1HipotUpdateManualNgMutation();
...
export const CompImage = ({ setAlert }) => {
    const selectedLogImage = useSelector(
        (state) => state.line1Hipot.selectedLogImage
    );
    useEffect(() => {
...
const TopAutoNgTable = ({ searchParams, setSearchParams }) => {
    ...
    } = useGetLine1HipotTop5NgCauseQuery(searchParams, {
        pollingInterval: interval,
    });
    return (
        <Table>
...
export const Hipot = () => {
    ...
    const { data: line1HipotCount = {}, isLoading: line1HipotCountLoading } =
        useGetLine1HipotCountQuery(searchParams, {
            pollingInterval: interval,
        });
    const {
